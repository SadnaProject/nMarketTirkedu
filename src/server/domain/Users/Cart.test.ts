...
describe("addProduct", () => {
    it("should add a product to the basket", () => {
        ...
        expect(cart.DTO.storeIdToBasket.get(storeId)?.products.filter((p) =>p.storeProductId===productId)[0]?.quantity).toBe(1);
        expect(cart.DTO.storeIdToBasket.get(storeId)?.products.length).toBe(1);
        cart.addProduct(productId, storeId, quantity + 1);
        expect(cart.DTO.storeIdToBasket.get(storeId)?.products.filter((p) =>p.storeProductId===productId)[0]?.quantity).toBe( quantity + 1);
...
describe("removeProduct", () => {
    it("should remove a product from the basket", () => {
        ...
        expect(cart.DTO.storeIdToBasket.get(storeId)?.products.filter((p) =>p.storeProductId===productId).length).toBe(0);
...
describe("editProductQuantity", () => {
    it("should edit the quantity of a product in the basket", () => {
        ...
        expect(cart.DTO.storeIdToBasket.get(storeId)?.products.filter((p) =>p.storeProductId===productId)[0]?.quantity).toBe( quantity);    
