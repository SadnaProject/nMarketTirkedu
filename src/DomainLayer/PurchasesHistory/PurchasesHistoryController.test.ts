...
import { BasketPurchase } from "./BasketPurchase";
...
describe("ProductReview constructor", () => {
    ...
    it("❎gets negative rating", () => {
        expect(() => new ProductReview({...productReviewData, rating : -1})).toThrow();
    });

    it("❎gets rating over 5", () => {
        expect(() => new ProductReview({...productReviewData, rating : 6})).toThrow();
    });
    it("❎gets storeId and productId", () => {
        expect(() => new ProductReview({...productReviewData, storeId : "storeId"})).toThrow();
    });
    it("❎gets undefined productId", () => {
        expect(() => new ProductReview({...productReviewData, productId : undefined})).toThrow();
    });
});
...
describe("addProductPurchaseReview", () => {
    it("❎adds product purchase review", () => {
        ...
        expect(() => purchasesHistoryController.addProductPurchaseReview("userId", "storeId", "purchaseId", "productId", {...productReviewData, rating : 5})).toThrow();
    });
});
...
describe("addStorePurchaseReview", () => {
    it("❎adds store purchase review", () => {
        ...
        const productReview = createProductReview();
        const purchasesHistoryController = new PurchasesHistoryController();
        expect(() => purchasesHistoryController.addStorePurchaseReview("userId", "storeId", "purchaseId", 5)).toThrow();
